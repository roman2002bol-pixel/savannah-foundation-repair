...
## 2026-09-25 – audit of the September 22-24 Codex pass (findings only, no site changes)

Roman asked for a full review of the Codex thread "Оціни три сайти". This entry records measured findings. **Nothing on the site was changed in this audit**; the recommendations at the end are proposals awaiting his decision.

**Confirmed correct, worth keeping.** Removing the "18 AREAS WRITTEN UP" stat (it counted pages, not customer value). Removing implied-experience wording from area pages that a business with no completed jobs cannot support -- the prior copy said "the two failure modes we see constantly downtown", "the single most common thing we see out here", "failures out here are predictable" and "what we find here is nearly always moisture-driven". Visible, underlined citation links. The scoped USDA Pooler soil-series source. Removing unconditional repair outcomes. Asking for real jobs before any case study and labelling examples as illustrative. The marker-based local-research renderer: repeated rendering leaves the researched block text identical on all 18 area pages.

**Measured regression: area pages became more templated.** Method: take each area page's `<main>` text, replace the town name with a placeholder, split into 6-word shingles, and compute average pairwise Jaccard overlap across the 18 pages. Before the pass (`b64d7a0^`): average 8.1%, maximum 11.4%, about 475 words per page. Now (`HEAD`): average 27.2%, maximum 58.4%, about 426 words. Causes: an identical lede template with only the place swapped, the same three FAQ questions on every page, and two of the three focus items identical on all 18.

**Researched facts removed with the overclaims.** In a 30-fact sample across 13 area pages, 28 are gone -- among them Tybee's base-flood-elevation requirement, FEMA mitigation grants and the Strand Cottages district; Windsor Forest; Garden City's 1939 origin; Thunderbolt's history; Fort Stewart; and two honest scope limits (no house elevation on Tybee; no HUD manufactured-home releveling in Bloomingdale). Distinct external sources on area pages fell from 22 to 13; the Chatham County flood-map page now appears on 9 of 18. Most of those facts were sourced and verifiable; only the experience and diagnosis-by-neighborhood wording needed removal.

**Generator and published-file drift.** Running all four builders in a correctly named checkout succeeds, but differs from the committed files in three ways: `llms.txt` still lists the old per-area focus claims that were removed from the pages (build_meta.py was not re-run after the area data changed), committed pages reference `main.js?v=20260922` while generators emit `20260924`, and sitemap dates. `content_review.py` identifies the site from the folder name; a checkout named differently falls through to the NEXUS branch and fails with `KeyError: 'concrete-slab-leveling'`. It also holds the NEXUS and Tree scenario content, although it exists only in this repository.

**Proposals, not implemented:** restore the verified per-area facts and scope limits with implied-experience wording removed; replace the templated lede, FAQ set and shared focus items with area-specific questions; re-run build_meta.py so llms.txt matches the pages; replace folder-name detection with an explicit site setting; add the overlap measurement as a check so this regression is caught automatically.
